...
<article class="page-container">
	<form class="form form-horizontal" onsubmit="return checkvolid();" id="form-admin-add" action="<%=path%>/room/addroom" method="post">
		<input type="hidden" name="room.buildingIdString" value="${room.buildingIdString}" id="buildid"/>
		<input type="hidden" name="huxing.idString" value="${huxing.idString}" id="huxingid"/>
		<input type="hidden" name="room.idString" value="${room.idString}"/>
		<div class="row cl">
			<label class="form-label col-xs-2 col-sm-3">楼栋户型：</label>
			<div class="formControls col-xs-4 col-sm-4"> 
				<span class="select-box">
					<select class="select" name="" onchange="gethuxing(this.value);" id="build_buildidString" size="1">
						<option value="-1">请选择楼栋</option>
						<c:forEach items="${loudonglist}" var="l">
							<c:if test="${l.idString==room.buildingIdString}">
								<option value="${l.idString}" selected="selected">${l.nameString}楼栋</option>
							</c:if>
							<c:if test="${l.idString!=room.buildingIdString}">
								<option value="${l.idString}">${l.nameString}楼栋</option>
							</c:if>
						</c:forEach>
					</select>
				</span> 
			...
			<div class="formControls col-xs-4 col-sm-4"> 
				<span class="select-box">
					<select class="select" name="room.houseIdString" id="room_buildidString" size="1">
						<c:forEach items="${houselist}" var="l">
							<c:if test="${room.houseIdString==l.idString}">
								<option value="${l.idString}" selected="selected">${l.nameString}户型</option>
							</c:if>
							<c:if test="${room.houseIdString!=l.idString}">
								<option value="${l.idString}">${l.nameString}户型</option>
							</c:if>
						</c:forEach>
					</select>
				</span> 
		...
		<div class="row cl">
			<label class="form-label col-xs-2 col-sm-3">房号名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" required  value="${room.nameString}" onblur="checkname(this.value);" placeholder="请输入房号名称" id="room_nameString" name="room.nameString">
				<span style="display:none;color:#ff0033;align:center;font-size:4px;height:8px" id="nameString" ></span>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-2 col-sm-3">房号简介：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="room.roominfoString"  required cols="" id="room_roominfoString" rows="" class="textarea"  placeholder="说点什么...500个字符以内" dragonfly="true" onKeyUp="$.Huitextarealength(this,500)">${room.roominfoString}</textarea>
