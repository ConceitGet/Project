...
<article class="page-container">
	<form action="<%=path %>/jxs/updatepwd" method="post" class="form form-horizontal" id="form-change-password">
		<div class="row cl">
...
function closenow(){
	...
	if(newpwd==""){
		...
		$.post(
			"${pageContext.request.contextPath}/jxs/initpwd",
			{
			},
			function(data){
				if(data!=newpwd){
					alert("newpwd="+newpwd+";data="+data);
					document.getElementById('pwd').style.display='block';
					$("#pwd").text("旧密码输入错误！");
					return;
				}else if(data==newpwd){
	...
	$.post(
		"${pageContext.request.contextPath}/jxs/updatepwd",
		{
