		...
		<table
			...
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.articleslist">
					<tr class="text-c">
						<td>
							<a href="<%=path %>/${l.contenturlString}"><img src="<%=path %>/${l.logoString}" style="width:4em;height:4em;border-radius:100%" alt="预览"/></a>
						</td>
						<td>
							<a href="<%=path %>/${l.contenturlString}">${l.titleString}</a>
						</td>
						<td>${l.createdTime}</td>
						<td>
						...
						<td class="td-status"><a href="<%=path %>/${l.contenturlString}" target="_blank">预览</a></td>
					</tr>
				</s:iterator>
			</tbody>
		</table>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/wz/allarticlelist?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/wz/allarticlelist?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/allarticlelist?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/wz/allarticlelist?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/wz/allarticlelist?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/wz/allarticlelist?currentpage='+index;
		}
		/*文章-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/wz/allarticlelist';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
...
// 			layer.full(index);
		...
		function article_del(obj, id) {
			...
			$.ajax({
					type : 'POST',
					url : '${pageContext.request.contextPath}/wz/deleteld?article.idString='+ id,
					dataType : 'text',
					success : function(data) {
						layer.msg('已删除!', {
							icon : 1,
