...
<article class="page-container">
	<form class="form form-horizontal" enctype="multipart/form-data" id="form-article-add" method="post" action="<%=path%>/manager/upfile">
		<div class="row cl">
...
<!-- 			</div> -->
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" required  placeholder="请输入文章标题" id="article_titleString" name="article.titleString">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>文章作者：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" required  placeholder="请输入文章作者" id="article_publicusernameString" name="article.publicusernameString">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">文章摘要：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="article.abstractsString" id="article_abstractsString" cols="" rows="" class="textarea"  placeholder="文章摘要...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="楼盘简介不能为空！" onKeyUp="checkabstracts(this.value);"></textarea>
				<p class="textarea-numberbar"><em class="textarea-length" id="abstracts">0</em>/220</p>
			</div>
		</div>
...
<script type="text/javascript">
	function checkabstracts(name){
		if(name.length<=220){
			$("#abstracts").text(name.length);
		}else if(name.length>220){
			layer.msg('最多220个字符!', {
				icon : 2,
				time : 2000
			});
			$("#article_abstractsString").val(name.substring(0,220));
			return;
		}
