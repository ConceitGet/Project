...
<article class="page-container">
	<form class="form form-horizontal" enctype="multipart/form-data" id="form-article-add" method="post" action="<%=path%>/manager/updasavetearticle">
		<input name="article.logoString" type="hidden" value="${article.logoString}" />\
		<input name="article.idString" type="hidden" value="${article.idString}" />
		<input name="article.contenturlString" type="hidden" value="${article.contenturlString}" />
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">原图：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<img src="<%=path %>/${article.logoString}" style="width:100px;height:100px" />
			</div>
		</div>
		<div class="row cl">
				...
				<span class="select-box">
					<select class="select" name="article.typeInt" id="article_typeInt" size="1">
						...
						<c:forEach items="${typelist}" var="l">
							<c:if test="${article.typeInt==l.idString}">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" required  value="${article.titleString}" placeholder="请输入文章标题" id="article_titleString" name="article.titleString">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>文章作者：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" required  value="${article.publicusernameString}" placeholder="请输入文章作者" id="article_publicusernameString" name="article.publicusernameString">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">文章摘要：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="article.abstractsString" id="article_abstractsString" cols="" rows="" class="textarea"  placeholder="个人简介...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="楼盘简介不能为空！" onKeyUp="$.Huitextarealength(this,200)">${article.abstractsString}</textarea>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> 
				<textarea name="article.contentString" id="editor" style="width:100%;height:400px;">${article.contentString}</textarea>
