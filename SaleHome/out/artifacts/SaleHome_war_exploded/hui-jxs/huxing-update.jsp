...
<![endif]-->
<title>${house.tbuilding.buildings.tagency.leaderString}经销商的${house.tbuilding.buildings.nameString}楼盘的第${house.tbuilding.nameString}楼栋的${house.nameString}户型的信息修改</title>
</head>
<body>
<article class="page-container">
	<form class="form form-horizontal" onsubmit="return checkvolid();" enctype="multipart/form-data" id="form-admin-add" action="<%=path%>/hx/updatesave" method="post">
		<input type="hidden" name="tbuilding.idString" value="${house.buildidString}" id="buildid"/>
		<input type="hidden" name="house.idString" value="${house.idString}"/>
		<input type="hidden" name="house.logoString" value="${house.logoString}"/>
		<input type="hidden" name="house.statusInt" value="${house.statusInt}"/>
		<input type="hidden" name="house.createdTime" value="${house.createdTime}"/>
		<div class="row cl">
			<label class="form-label col-xs-2 col-sm-3">户型图片：</label>
			<div class="formControls col-xs-4 col-sm-5"> 
				<img src="<%=path %>/${house.logoString}" style="width:15em;height:3em"/>
			</div>
			<label class="form-label col-xs-1 col-sm-1"></label>
			<div class="formControls col-xs-4 col-sm-5"> <span class="btn-upload form-group">
				...
				<span class="select-box">
					<select class="select" name="loupan.idString" onchange="getld(this.value);" id="house_buildingsId" size="1">
						...
						<c:forEach items="${loupanlist}" var="l">
							<c:if test="${house.tbuilding.buildings.idString==l.idString}">
			...
			<div class="formControls col-xs-4 col-sm-4"> 
				<span class="select-box">
					<select class="select" name="house.buildidString" id="house_buildidString" size="1">
						<c:forEach items="${loudonglist}" var="l">
							<c:if test="${house.buildidString==l.idString}">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" required  value="${house.nameString}" onblur="checkname(this.value);" placeholder="请输入户型名称" id="house_nameString" name="house.nameString">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="number" class="input-text" required  style="width:26.5em" autocomplete="off" value="${house.areaDouble}" placeholder="请输入户型总层数" onblur="checktotal(this.value);" id="house_areaDouble" name="house.areaDouble">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="number" class="input-text" value="${house.unitPriceDouble}" style="width:26.5em"  autocomplete="off"  onblur="checklift(this.value);" placeholder="请输入户型电梯总数" id="house_unitPriceDouble" name="house.unitPriceDouble">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="house.introString" cols="" id="house_introString" rows="" class="textarea"  placeholder="说点什么...100个字符以内" dragonfly="true" onKeyUp="$.Huitextarealength(this,100)">${house.introString}</textarea>
...
<script type="text/javascript">
	...
	function checkvolid(){
		...
		if($("#house_totalFloorInt").val()==''){
			document.getElementById('total').style.display='block';
			$("#total").text("户型层数不能为空");
			volidvalue=false;
		}else if($("#house_totalFloorInt").val()!=''){
			document.getElementById('total').style.display='none';
			volidvalue=true;
		}
		if($("#house_totalLiftInt").val()==''){
			document.getElementById('lift').style.display='block';
			$("#lift").text("户型电梯总数不能为空");
			volidvalue=false;
		}else if($("#house_totalLiftInt").val()!=''){
			document.getElementById('lift').style.display='none';
			volidvalue=true;
		}
		if($("#house_floorRoomsInt").val()==''){
			document.getElementById('floor').style.display='block';
			$("#floor").text("户型电梯总数不能为空");
			volidvalue=false;
		}else if($("#house_floorRoomsInt").val()!=''){
			document.getElementById('floor').style.display='none';
			volidvalue=true;
		}
