...
<body>
	<nav class="breadcrumb">
		...
		我的预约管理 <span class="c-gray en">&gt;</span> 我的预约列表 <a
	...
	<div class="page-container">
		...
		<table
			...
			<thead>
				<tr>
					<th scope="col" colspan="10">我的预约列表</th>
				</tr>
				<tr class="text-c">
			...
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.appointmentlist">
					<tr class="text-c">
						...
						<td>
							<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="user_edit('${l.user.nameString}我的预约详情','<%=path%>/user/showuser?user.idString=${l.user.idString}','${l.user.nameString}','','530')">${l.user.nameString}</a>
						</td>
						<td>
						...
						<c:if test="${l.status==0}">
							<td class="td-status"><span class="label label-success radius">预约成功</span></td>
							<td class="td-manage">
								<a style="text-decoration: none"	onClick="appointment_stop(this,'${l.idString}')" href="javascript:;"	title="禁用"><i class="Hui-iconfont">&#xe631;</i></a> 
								<c:if test="${l.appStatusInt==0}">
									<a title="编辑" href="javascript:;"	onclick="appointment_edit('${l.user.nameString}我的的修改','<%=path%>/hui-jxs/update-userappointmentstatus.jsp?idString=${l.idString}&name=${l.employee.nameString}&email=${l.employee.emailString}&house=${l.house.tbuilding.buildings.nameString}楼盘${l.house.tbuilding.nameString}楼栋的${l.house.nameString}户型&username=${l.user.nameString}','${l.user.nameString}','600','360')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a> 
								</c:if>
								<c:if test="${l.appStatusInt==1}">
									<a title="编辑" href="javascript:;"	onclick="appointment_edit('${l.user.nameString}我的的修改','<%=path%>/hui-jxs/update-userappointmentstatus.jsp?idString=${l.idString}&name=${l.employee.nameString}&email=${l.employee.emailString}&house=${l.house.tbuilding.buildings.nameString}楼盘${l.house.tbuilding.nameString}楼栋的${l.house.nameString}户型&username=${l.user.nameString}','${l.user.nameString}','600','360')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a> 
								</c:if>
						...
						<c:if test="${l.status==1}">
							<td class="td-status"><span class="label label-default radius">已取消预约</span></td>
							<td class="td-manage">
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/appointment/pageuserappointment?currentpage=1&handle=firstpage">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/appointment/pageuserappointment?currentpage=${pager.currentpage}&handle=uppage">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pageuser?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/appointment/pageuserappointment?currentpage=${pager.currentpage}&handle=downpage">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/appointment/pageuserappointment?currentpage=${pager.sumpage}&handle=lastpage">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/appointment/pageuserappointment?currentpage='+index;
		}
		/*我的预约-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/appointment/pageuserappointment';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
...
// 			layer.full(index);
		...
		function appointment_stop(obj, id) {
			layer.confirm('确认禁用吗？',function(index) {
				...
				$(obj).parents("tr").find(".td-status").html('<span class="label label-default radius">已取消预约</span>');
				$(obj).remove();
				layer.msg('已取消预约!', {
					icon : 5,
					time : 1000
