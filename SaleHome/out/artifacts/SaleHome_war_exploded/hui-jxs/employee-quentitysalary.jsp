...
<body>
	...
	<h2>基本工资：<font color="red" size="28">￥<%=employee.getBasicSalaryDouble() %></font></h2>
		...
		<table
			...
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.userprizelist">
					<tr class="text-c">
						<td>
							<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="loupan_edit('${l.customer.nameString}用户详情','<%=path%>/customer/showcustomer?customer.idString=${l.userIdString}','${l.customer.nameString}','','430')">${l.customer.nameString}</a>
						</td>
						<td>
							${l.prize}
						</td>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/userprize/quentitysalary?currentpage=1&handle=firstpage&tbuilding.idString=${tbuilding.idString}">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/userprize/quentitysalary?currentpage=${pager.currentpage}&handle=uppage&tbuilding.idString=${tbuilding.idString}">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pagehouse?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/userprize/quentitysalary?currentpage=${pager.currentpage}&handle=downpage&tbuilding.idString=${tbuilding.idString}">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/userprize/quentitysalary?currentpage=${pager.sumpage}&handle=lastpage&tbuilding.idString=${tbuilding.idString}">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/userprize/quentitysalary?currentpage='+index;
		}
		function loudong(){
		...
		/*出单记录-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/userprize/quentitysalary?tbuilding.idString=${tbuilding.idString}';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
