...
<body>
	<nav class="breadcrumb">
		...
		出单记录管理 <span class="c-gray en">&gt;</span> 出单记录列表 <a
			class="btn btn-success radius r"
			style="line-height: 1.6em; margin-top: 3px"
			href="javascript:void(0);" onclick="refresh();" title="刷新"><i
			class="Hui-iconfont">&#xe68f;</i></a>
	</nav>
	<h2>销售收入：<font color="red" size="28">￥${mymoney/10000}万元+￥${employee.basicSalaryDouble}=￥${(mymoney+employee.basicSalaryDouble)/10000}万元</font></h2>
	<div class="page-container">
		...
		<table
			...
			<thead>
				<tr>
					<th scope="col" colspan="3">出单记录列表</th>
				</tr>
				<tr class="text-c">
					<th width="150">出单对象</th>
					<th width="90">出单收入</th>
					<th width="150">出单时间</th>
				</tr>
			</thead>
			<tbody id="pageBody">
				<s:iterator var="l" value="#request.userprizelist">
					<tr class="text-c">
						<td>
							<a style="cursor: pointer;text-decoration: none;" class="text-primary" onclick="loupan_edit('${l.customer.nameString}客户详情','<%=path%>/customer/showcustomer?customer.idString=${l.userIdString}','${l.customer.nameString}','700','430')">${l.customer.nameString}</a>
						</td>
						<td>
							${l.prize}
						</td>
			...
			<div class="row-fluid">
				...
				<div class="span6">
					<div class="dataTables_paginate paging_bootstrap pagination">
						<ul>
							<li class="prev">
								<a href="<%=path%>/userprize/mysumsalary?currentpage=1&handle=firstpage&tbuilding.idString=${tbuilding.idString}">← 第一页</a>
							</li>
							<li class="prev">
								<a href="<%=path%>/userprize/mysumsalary?currentpage=${pager.currentpage}&handle=uppage&tbuilding.idString=${tbuilding.idString}">← 上一页</a>
							</li>
<!-- 							<li> -->
<%-- 								<a href="<%=path%>/lp/pagehouse?currentpage=${n}">${n}</a> --%>
<!-- 							</li> -->
							<li class="next">
								<a href="<%=path%>/userprize/mysumsalary?currentpage=${pager.currentpage}&handle=downpage&tbuilding.idString=${tbuilding.idString}">下一页 → </a>
							</li>
							<li class="next">
								<a href="<%=path%>/userprize/mysumsalary?currentpage=${pager.sumpage}&handle=lastpage&tbuilding.idString=${tbuilding.idString}">尾页 → </a>
							</li>
						</ul>
					</div>
				</div>
...
<script type="text/javascript">
		...
		function checkpage(index){
			window.location.href='${pageContext.request.contextPath}/userprize/mysumsalary?currentpage='+index;
		}
		function loudong(){
		...
		/*出单记录-刷新*/
		function refresh(){
			window.location.href='${pageContext.request.contextPath}/userprize/mysumsalary?tbuilding.idString=${tbuilding.idString}';
			layer.msg('刷新成功!', {
				icon : 1,
				time : 2000
			});
