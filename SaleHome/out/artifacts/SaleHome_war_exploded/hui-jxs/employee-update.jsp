	...
	<form class="form form-horizontal" enctype="multipart/form-data" onsubmit="return checkvolid();" id="form-admin-add" action="<%=path%>/yg/saveemployeeinfo" method="post">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" required  value="${employee.nameString}" onblur="checkname(this.value);" placeholder="请输入正实姓名" id="employee_nameString" name="employee.nameString">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="email" class="input-text" required  value="${employee.emailString}" onblur="checkemailString(this.value);" placeholder="请输入用户邮箱" id="employee_emailString" name="employee.emailString">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="number" class="input-text" required  value="${employee.phoneString}" onblur="checkphone(this.value);" placeholder="请输入用户手机号码" id="employee_phoneString" name="employee.phoneString" style="width:59em">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text"  required value="${employee.wechatString}" onblur="checkwechatString(this.value);" placeholder="请输入用户微信账号" id="employee_wechatString" name="employee.wechatString">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="number" class="input-text" required  value="${employee.qqString}" onblur="checkqq(this.value);" placeholder="请输入用户QQ号码" id="employee_qqString" name="employee.qqString"  style="width:59em">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="employee.introString" id="employee_introString" cols="" rows="" class="textarea"  placeholder="个人简介...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="楼盘简介不能为空！" onKeyUp="checkinfo(this.value);">${employee.introString}</textarea>
				<p class="textarea-numberbar"><em class="textarea-length" id="info">0</em>/200</p>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-2 col-sm-3">工作经验：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="employee.expString" id="employee_expString" cols="" rows="" class="textarea"  placeholder="工作经验简介...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="楼盘简介不能为空！" onKeyUp="checkexp(this.value);">${employee.expString}</textarea>
				<p class="textarea-numberbar"><em class="textarea-length" id="exp">0</em>/200</p>
			</div>
		</div>
		<div class="row cl">
...
<script type="text/javascript">
	$(function(){
		var num ="${employee.introString}";
		$("#info").text(num.length);
		var num1 ="${employee.expString}";
		$("#exp").text(num1.length);
	});
</script>
<script type="text/javascript">
function checkinfo(name){
	if(name.length<=200){
		$("#info").text(name.length);
	}else if(name.length>200){
		layer.msg('最多200个字!', {
			icon : 2,
			time : 1000
		});
		$("#employee_infoString").val(name.substring(0,200));
		return;
	}
}
</script>
<script type="text/javascript">
	function checkexp(name){
		if(name.length<=200){
			$("#exp").text(name.length);
		}else if(name.length>200){
			layer.msg('最多200个字!', {
				icon : 2,
				time : 1000
			});
			$("#employee_expString").val(name.substring(0,200));
			return;
		}
	}
</script>
